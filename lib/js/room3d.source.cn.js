...
(function(gloal){
  ...
  Box3D.prototype = {
    ...
    destroy(){
      ...
      //删除内部引用
      this.father.roomWraper.removeChild(this.boxWraper)//删除dom
      this.boxWraper = null
      this.father = null
      this.options = null
    }
  ...
  function Marker3D (scene,box,options){
    ...
    //计算marker旋转角度使其永远朝向屏幕
    faceMe(
      marker_this.scene.options.rotateVer,
      marker_this.scene.options.rotateHor,
      content
    )
    //创建连线
  ...
  Marker3D.prototype = {
    destroy(){//销毁标注
      ...
      this.scene.markers.splice(index)//从scene对象中删除引用
      this.box.marker = null//将box对象中的引用删除
      //删除内部引用
      this.scene.sceneGround.removeChild(this.wraper)//从场景中删除dom
      this.wraper = null
